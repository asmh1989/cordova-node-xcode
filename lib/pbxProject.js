...
pbxProject.prototype.addToFrameworkSearchPaths = function(file) {
    ...
    for (config in configurations) {
        ...
        if(typeof buildSettings['FRAMEWORK_SEARCH_PATHS'] == 'string' ){
					buildSettings['FRAMEWORK_SEARCH_PATHS'] = [buildSettings['FRAMEWORK_SEARCH_PATHS']];
        }

        let willAddPath = searchPathForFile(file, this);
        let willAdd = true;
        for(let p of buildSettings['FRAMEWORK_SEARCH_PATHS']){
            if(p == willAddPath){
                willAdd = false;
            }
        }
        ...
        if(willAdd){
					buildSettings['FRAMEWORK_SEARCH_PATHS'].push(willAddPath);
				}
